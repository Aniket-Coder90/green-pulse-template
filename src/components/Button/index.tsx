...
export const Button = ({ children, type, className = 'primary', style }: IProps) => {
    ...
    return (
        <button type={type} style={style} className={(className.includes('outlined') ? buttonClass + ' border-primary border-2 text-primary' : buttonClass) + ' w-[fit-content] overflow-hidden relative inline-flex items-center gap-[10px] p-[15px]'}>
            <span>{children}</span>
